...
std::vector<SurfaceSignal> ChargeDrift::Drift(std::vector<EnergyFluctUnit> EnergyLossVector)
{
    ...
    for (int k = 0; k < EnergyLossVector.size(); k++) {
        ...
        for (int i = 0; i < DriftPos.size(); i++) {
            ...
            Double_t tn = (stripsensor::drift::module_thickness * stripsensor::drift::module_thickness)
                          / (2 * stripsensor::drift::depletion_voltage * stripsensor::drift::charge_mobility);
            DriftTime = -tn
                            * log(1
                                  - 2 * stripsensor::drift::depletion_voltage * DriftDistanceFraction
                                        / (stripsensor::drift::depletion_voltage + stripsensor::drift::applied_voltage))
                        + stripsensor::drift::chargedistributionRMS;
            ...
            DiffusionConstant =
                (1.38E-23 / 1.6E-19) * stripsensor::drift::charge_mobility * stripsensor::drift::temperature;
        ...
        // Returning the surface signal with the area of the diffusion, position at surface and the total number of
        // electrons
